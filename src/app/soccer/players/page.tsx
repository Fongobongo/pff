...
export default async function SoccerPlayersPage({
  searchParams,
}: {
  ...
  return (
    <SoccerPageShell title="Soccer players" description="Sport.fun soccer token directory and pricing.">
      ...
      <section className="mt-6 flex flex-wrap gap-2 text-sm text-zinc-600 dark:text-zinc-400">
        {Array.from({ length: totalPages }, (_, idx) => idx + 1).slice(0, 8).map((p) => (
          <Link
            ...
            href={`/soccer/players${buildQuery({ q: params.q, sort, page: String(p) })}`}
          >
            {p}
          </Link>
        ))}
