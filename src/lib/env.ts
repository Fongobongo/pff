...
const envSchema = z.object({
  ...
  // Legacy/typo alias for Alchemy key (kept for compatibility).
  ALEMBIC_API_KEY: z.string().min(1).optional(),
});
...
const parsed = envSchema.parse({
  ...
  ALCHEMY_API_KEY: process.env.ALCHEMY_API_KEY ?? process.env.ALEMBIC_API_KEY,
  ALEMBIC_API_KEY: process.env.ALEMBIC_API_KEY,
});

export const env = {
  ...parsed,
  ALCHEMY_API_KEY: parsed.ALCHEMY_API_KEY ?? parsed.ALEMBIC_API_KEY,
